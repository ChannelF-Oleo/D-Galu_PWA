...
// Ejecutar con: node fix-admin-permissions.js
...
async function fixAdminPermissions() {
  try {
    ...
    const testNotification = await notificationsRef.add({
      title: 'Sistema Actualizado',
      message: 'Los permisos de administrador han sido corregidos.',
      type: 'system',
      targetRoles: ['admin'],
      targetUsers: [adminUid],
      isRead: false,
      createdAt: admin.firestore.FieldValue.serverTimestamp(),
      createdBy: adminUid
    });
    
    console.log(`📢 Notificación de prueba creada: ${testNotification.id}`);
    
    // Crear un log del sistema
    const systemLogsRef = admin.firestore().collection('system_logs');
    await systemLogsRef.add({
      type: 'admin_permissions_fixed',
      userId: adminUid,
      message: 'Permisos de administrador corregidos exitosamente',
      timestamp: admin.firestore.FieldValue.serverTimestamp(),
      metadata: {
        userEmail: adminEmail,
        customClaimsUpdated: true,
        userDocumentUpdated: true
      }
    });
